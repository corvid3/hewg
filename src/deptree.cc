...
struct DeptreeCtx {
  ...
  */
  ...
  auto
  collect_static_domain(PackageIdentifier const& ident)
  {
  ...
  auto
  collect_packages_by_name_in_static_domain(
  ...
  auto
  collect_packages_to_include(PackageIdentifier const& owner_ident)
  {
    return interpreter.query(
  ...
  */
  ...
  void
  add_package(std::string_view  org,
  ...
  void
  add_package(PackageIdentifier const& ident, PackageType const package_type)
  {
  ...
  void
  add_dependency(PackageIdentifier const&    parent,
  ...
  */
...

  {
    for (auto const& subst :
      ...
      auto const versions = ctx->collect_packages_by_name_in_static_domain(
        this_package_identifier,
        subst.at("ORG"_V),
        subst.at("NAME"_V),
        subst.at("TARGET"_V));
...

  {
    ...
    */
    ...
    for (auto const& subst : greater_versions) {
      ...
      if (semver->major() == 0) {
        throw std::runtime_error(
          "a dependent package with major version 0 may "
          "not be a >= dependency");
      }
    }
  }
  ...
  -> std::set<PackageIdentifier>
{
  ...
  for (auto const& subst :
       pkg.tree()->collect_packages_to_include(this_package_identifier)) {
  ...
  -> std::set<PackageIdentifier>
{
  ...
  for (auto const& subst :
       pkg.tree()->collect_static_domain(this_package_identifier)) {
    auto const org     = subst.at("ORG"_V);
    auto const name    = subst.at("NAME"_V);
    auto const version = subst.at("VERSION"_V);
    auto const target  = subst.at("TARGET"_V);

    auto const package_query = pkg.tree()->interpreter.query(
