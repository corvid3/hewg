...
struct VersionManifest
{
  ...
  using jayson_fields =
    std::tuple<jayson::obj_field<"versions", &VersionManifest::versions>>;
};
...
{
  if (not std::filesystem::create_directory(user_hewg_directory))
    throw std::runtime_error("unable to create user hewg directory");
...
static std::fstream
open_version_manifest_stream(std::string_view package_name)
{
  auto const dir = packages_directory / package_name;
  auto const version_manifest_file_path = dir / "versions.json";

  if (not std::filesystem::exists(dir))
    throw std::runtime_error("attempting to open a version manifest in a "
                             "package directory that doesn't exist");

  return std::fstream(version_manifest_file_path);
}
                       ...
                       version_triplet const version_triplet)
{
  ...
  auto const ver_json = jayson::serialize(version_triplet);
  auto version_manifest = open_version_manifest_stream(package_name);

  std::stringstream version_data;
  version_data << version_manifest.rdbuf();

  VersionManifest manifest;
  jayson::val manifest_json = jayson::val::parse(version_data.str());
  jayson::deserialize(manifest_json, manifest);
  ...
  manifest_json = jayson::serialize(manifest);
  version_manifest << manifest_json.serialize(true);
                   ...
                   std::filesystem::path const& install_dir)
{
  ...
  std::filesystem::copy_file(executable_path, install_dir / executable_name);
...
void
install(ConfigurationFile const& config, std::string_view profile)
{
  ensure_user_hewg_directory();
